        ...
        <div className="space-y-2">
          ...
          {nextSpeakers.length > 0 && (
            <div className="flex items-center gap-2 mt-4">
              ...
              <Badge variant="outline" className="font-medium">{nextSpeakers[0].name}</Badge>
              {nextSpeakers.length > 1 && <span className="text-xs text-muted-foreground">+{nextSpeakers.length - 1} more</span>}
            </div>
          )}
        </div>
        
        <Button size="lg" onClick={onNextSpeaker} className="shadow-lg hover:shadow-xl transition-all duration-200 group">
          Next Speaker
